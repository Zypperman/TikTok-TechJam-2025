...
class UnifiedDatasetProcessorV2:
    ...
    def process_google_dataset(
        self, csv_path: str = "google_dataset/kaggle/reviews.csv"
    ) -> pd.DataFrame:
        """
    ...
    def process_mcauleylab_dataset(
        self,
        review_path: str = "review-Alabama.json.gz",
        meta_path: str = "google_dataset/google/meta-Alabama.json.gz",
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            print(
                f"🔍 Found {len(reviews_data)} reviews and {len(business_data)} businesses"
            )
            ...
            df["business_category"] = df["business_id"].map(
                lambda x: (
                    business_data.get(x, {}).get("category", ["Unknown"])[0]
                    if business_data.get(x, {}).get("category")
                    else "Unknown"
                )
            )
    ...
    def merge_datasets(
        self, google_df: pd.DataFrame, mcauleylab_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        if not mcauleylab_df.empty:
            ...
            mcauleylab_unified = mcauleylab_df[
                [
                    "review_id",
                    "business_name",
                    "author_name",
        ...
        irrelevant_mask = text_lower.str.contains(
            "|".join(irrelevant_keywords), na=False
        )
        merged_df.loc[
            irrelevant_mask & (merged_df["policy_violation"] == "valid"),
            "policy_violation",
        ] = "irrelevant"
        ...
        merged_df.loc[
            rant_mask & (merged_df["policy_violation"] == "valid"), "policy_violation"
        ] = "rant"

        # Add confidence scores
        merged_df["violation_confidence"] = 0.0
        merged_df.loc[
            merged_df["policy_violation"] != "valid", "violation_confidence"
        ] = 0.7
    ...
    def save_unified_dataset(self, df: pd.DataFrame) -> None:
        ...
        ml_df["has_url"] = ml_df["text"].str.contains(
            r"http[s]?://", regex=True, na=False
        )
        ml_df["has_phone"] = ml_df["text"].str.contains(
            r"\b\d{3}[-.]?\d{3}[-.]?\d{4}\b", regex=True, na=False
        )
        ml_df["has_email"] = ml_df["text"].str.contains(
            r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b", regex=True, na=False
        )
        ml_df["exclamation_count"] = ml_df["text"].str.count(r"\!")
        ml_df["question_count"] = ml_df["text"].str.count(r"\?")
        ml_df["capitalization_ratio"] = ml_df["text"].apply(
    ...
    def generate_summary(self, df: pd.DataFrame) -> None:
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            ...
            for violation, count in violation_counts.items():
                f.write(
                    f"  {violation}: {count:,} reviews ({count/len(df)*100:.1f}%)\n"
                )
